#  iterative bidding and scheduling algorithm with priority feature
...
class EVChargingMarketSimulator:
    """
    Enhanced Electric Vehicle Charging Market Simulator with Emergency Vehicle Prioritization
    
    Features:
    - Multiple user behavior profiles including emergency vehicles
    ...
    def __init__(self, 
        ...
        # Market metrics tracking with priority allocation
        self.market_history = {
            'price': [],
            'demand': [],
            'supply': [],
    ...
    def _initialize_simulation(self):
        ...
        # Generate users with realistic profiles including emergency vehicles
    ...
    def _generate_charging_stations(self):
        ...
        for region, params in regions.items():
            ...
            for _ in range(count):
                ...
                # Create station with emergency priority slots (10% of capacity)
                emergency_slots = max(1, int(capacity * 0.1))
                station = {
                    'station_id': id_counter,
                    'region': region,
                    'capacity': capacity,
                    'emergency_capacity': emergency_slots,
        ...
        print(f"Emergency priority slots: {sum(s['emergency_capacity'] for s in stations)}")
    
    def _generate_users(self):
        """Generate realistic EV user profiles including emergency vehicles"""
        print("Generating user profiles...")
        
        # Define user archetypes with emergency vehicles
        archetypes = [
            # Emergency vehicles (2% of total)
            {
                'name': 'Emergency Vehicle',
                'probability': 0.02,
                'battery_capacity': (80, 120),
                'avg_daily_use': (100, 200),
                'charging_pattern': 'emergency',
                'time_sensitivity': 'very_high',
                'price_sensitivity': (0.1, 0.3),
                'range_anxiety': (0.8, 1.0),
                'priority': 1
            },
        ...
        if diff != 0:
            archetype_counts[archetypes[1]['name']] += diff  # Adjust Daily Commuter count
        ...
        print(f"Emergency vehicles: {sum(1 for u in users if u['priority'] == 1)}")
    ...
    def _generate_archetype_users(self, archetype: Dict, count: int, start_id: int) -> List[Dict]:
        ...
        for i in range(count):
            ...
            if archetype['charging_pattern'] == 'emergency':
                # Emergency vehicles need immediate charging at random times
                preferred_times = []
                for day in range(self.time_window_days):
                    if np.random.random() < 0.3:  # 30% chance of needing charge each day
                        hour = np.random.randint(0, 24)  # Any hour
                        preferred_times.append(day * 24 + hour)
            
            elif archetype['charging_pattern'] == 'routine':
            ...
            if archetype['name'] == 'Emergency Vehicle':
                archetype_factor = 1.5  # Emergency vehicles have budget for charging
            ...
            if archetype['name'] in ['Emergency Vehicle', 'Road Tripper'] or time_sensitivity > 0.7:
            ...
            elif archetype['name'] == 'Daily Commuter':
    ...
    def _train_price_model(self):
        ...
        for user in self.users:
            features = [
                ...
                user['priority']  # Include priority in features
            ]
    ...
    def _save_datasets(self):
        ...
        for station in self.stations:
            station_record = {
                ...
                'emergency_capacity': station['emergency_capacity'],
    ...
    def _allocate_charging_sessions(self, round_num: int) -> List[Dict]:
        """Match users to charging stations using priority-first allocation"""
        ...
        # Sort bids by priority (descending), then price (descending), then utility (descending)
        all_bids.sort(key=lambda x: (-x['priority'], -x['price'], -x['utility']))
        ...
        for station in self.stations:
            ...
            station_allocations[station_id] = {
                ...
                'priority_allocations': 0  # Count of priority allocations
            }
        
        # Track transactions and priority allocations
        transactions = []
        priority_counts = {'emergency': 0, 'normal': 0}
        
        # Allocate in order of priority then price
        for bid in all_bids:
            user_id = bid['user_id']
            hour = bid['hour']
            price = bid['price']
            ...
            for station_id in station_prefs:
                ...
                current_allocations = len(station['allocation_schedule'][hour])
                
                # For priority bids, check emergency capacity first
                if is_priority:
                    # Emergency vehicles can use any slot, but prefer dedicated emergency slots
                    if current_allocations < station['capacity']:
                        allocated_station = station
                        break
                else:
                    # Regular users can only use non-emergency slots
                    if current_allocations < (station['capacity'] - station['emergency_capacity']):
                        allocated_station = station
                        break
            ...
            if allocated_station:
                ...
                if profit > 0 or round_num > self.max_rounds * 0.7:
                    ...
                    if is_priority:
                        station_allocations[station_id]['priority_allocations'] += 1
        ...
        # Store priority allocation metrics
        total_allocations = sum(priority_counts.values())
        if total_allocations > 0:
            self.market_history['priority_allocation'].append({
                'round': round_num,
                'emergency_pct': priority_counts['emergency'] / total_allocations,
                'normal_pct': priority_counts['normal'] / total_allocations,
                'emergency_count': priority_counts['emergency'],
                'total_count': total_allocations
            })
        
        print(f"Completed {len(transactions)} charging transactions")
        print(f"Emergency allocations: {priority_counts['emergency']}")
        return transactions
    ...
    def _print_round_summary(self, round_num: int, transactions: List[Dict]):
        ...
        # Get priority allocation stats if available
        priority_stats = ""
        if self.market_history['priority_allocation'] and idx < len(self.market_history['priority_allocation']):
            p_alloc = self.market_history['priority_allocation'][idx]
            priority_stats = f" | Emergency: {p_alloc['emergency_count']}/{p_alloc['total_count']} ({p_alloc['emergency_pct']:.1%})"
        
        print(f"Round {round_num + 1} Summary:")
        print(f"- Transactions: {len(transactions)}{priority_stats}")
    ...
    def _calculate_user_utility(self, user: Dict) -> float:
        ...
        # Priority bonus for emergency vehicles
        priority_bonus = 20 if user['priority'] == 1 else 0
        
        # Calculate final score
        utility = max(0, min(100, 
            (charge_utility - price_penalty + time_bonus + priority_bonus) * anxiety_factor))
    ...
    def _finalize_simulation(self) -> Dict:
        ...
        total_utility = 0
        satisfied_users = 0
        emergency_users = [u for u in self.users if u['priority'] == 1]
        regular_users = [u for u in self.users if u['priority'] == 0]
        ...
        # Calculate emergency vehicle specific metrics
        emergency_satisfaction = 0
        if emergency_users:
            emergency_satisfaction = sum(u['utility_score'] for u in emergency_users) / len(emergency_users)
        ...
        # Priority allocation metrics
        priority_stats = {
            'emergency_users': len(emergency_users),
            'emergency_satisfaction': emergency_satisfaction,
            'emergency_allocations': sum(a['emergency_count'] for a in self.market_history['priority_allocation']),
            'total_allocations': sum(a['total_count'] for a in self.market_history['priority_allocation']),
            'emergency_pct': sum(a['emergency_count'] for a in self.market_history['priority_allocation']) / 
                            max(1, sum(a['total_count'] for a in self.market_history['priority_allocation']))
        }
    ...
    # Demand and transactions subplot
        ...
        # 4. Priority allocation visualization - FIXED VERSION
        if self.market_history['priority_allocation']:
            plt.figure(figsize=(12, 6))
            df = pd.DataFrame(self.market_history['priority_allocation'])

            # Create stacked bar plot instead of fill_between
            plt.bar(df['round'], df['emergency_pct']*100, label='Emergency Vehicles', color='#e74c3c')
            plt.bar(df['round'], df['normal_pct']*100, bottom=df['emergency_pct']*100, 
                    label='Regular Users', color='#3498db')

            plt.title('Priority-based Charging Allocation', fontsize=14)
            plt.xlabel('Round', fontsize=12)
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(f"{self.graph_dir}/priority_allocation.png", dpi=300)
    
    # [Rest of the visualization code remains the same...]
        
        # 5. Station utilization heatmap
        ...
        # 6. User archetype performance
        user_df = pd.DataFrame([{
            'archetype': u['archetype'],
            'priority': u['priority'],
            'final_charge': u['final_charge'],
            'battery_capacity': u['battery_capacity'],
            'utility_score': u['utility_score'],
            'total_cost': u['total_cost']
        } for u in self.users])
        
        archetype_performance = user_df.groupby(['archetype', 'priority']).agg({
            'final_charge': 'mean',
            'utility_score': 'mean',
            'total_cost': 'mean',
        ...
        archetype_performance['charge_pct'] = archetype_performance['final_charge'] / archetype_performance['battery_capacity'] * 100
        
        plt.figure(figsize=(14, 8))
        
        # Charge percentage by archetype
        plt.subplot(1, 2, 1)
        colors = ['#e74c3c' if p == 1 else '#3498db' for p in archetype_performance['priority']]
        ...
        # Add custom legend for priority
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor='#e74c3c', label='Emergency Vehicle'),
            Patch(facecolor='#3498db', label='Regular User')
        ]
        ...
        # 7. Pricing strategy effectiveness (revenue vs. price)
...
if __name__ == "__main__":
    ...
    print("\n=== Priority Statistics ===")
    print(f"Emergency vehicles: {results['priority_stats']['emergency_users']}")
    print(f"Emergency allocations: {results['priority_stats']['emergency_allocations']} ({results['priority_stats']['emergency_pct']:.1%} of total)")
    print(f"Emergency vehicle satisfaction: {results['priority_stats']['emergency_satisfaction']:.1f}/100")